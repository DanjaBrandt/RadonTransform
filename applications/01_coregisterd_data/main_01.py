...
from utils_01 import process_data

from display_handler_01 import DisplayCoregisterdImages
...
class MainClass:
    ...
    def __init__(self, config, process_key, **kwargs):
        ...
        if process_key == "display":
            self.processor = DisplayCoregisterdImages(**kwargs).display
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Choose the group to display. If None all mice will be displayed."
    )
    ...
    parser.add_argument(
        "--colors",
        help="If true magenta and green used for the channels, else default/grayscale."
    )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:  # CLI mode
        ...
        args = argparse.Namespace(
            mode="display",
            input_folder="01_data",
            result_folder="01_outputs/01_radon_output_00",
            display_mode="count_points",
            coregistered_channels=["2PM", "SHG"],
    ...
    app = MainClass(
        ...
        mouse=args.mouse,
        day=args.day,
        coregistered_channels=args.coregistered_channels,
        visualize_channel=args.visualize_channel,
        color=args.color,
        lines=args.lines,
